...
╚═╝  ╚═╝╚═════╝ ╚═════╝     ╚═╝  ╚═══╝ ╚═════╝ ╚═════╝ ╚══════╝
    `);
    console.log(chalk.blue(`Time: ${getTimestamp()}`));
    console.log(chalk.blue("---------------------------------------------"));
    console.log(
      chalk.blue(`User: ${userData.email || "N/A"} | ID: ${userData.id || "N/A"} | Referral Code: ${userData.referral_code || "N/A"}`)
    );
    console.log(chalk.blue("---------------------------------------------"));
    console.log(chalk.blue("VALIDATION STATISTICS:"));
    console.log(chalk.green(`✓ Valid Validations: ${userData.stats.stork_signed_prices_valid_count || 0}`));
    console.log(chalk.yellow(`✗ Invalid Validations: ${userData.stats.stork_signed_prices_invalid_count || 0}`));
    console.log(chalk.magenta(`↻ Last Validated At: ${userData.stats.stork_signed_prices_last_verified_at || "Never"}`));
    console.log(chalk.blue(`👥 Referral Usage Count: ${userData.stats.referral_usage_count || 0}`));
    console.log(chalk.blue("---------------------------------------------"));
    console.log(chalk.blue(`Next validation in ${config.stork.intervalSeconds} seconds...`));
    console.log(chalk.blue("============================================="));
  }
  ...
  async function main() {
    log("\nTool modified by telegram group (https://t.me/airdropbombnode)\n", "WARN");
    if (!validateConfig()) {
      process.exit(1);
    }
